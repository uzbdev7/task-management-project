    ...
    try {
        ...
        await db.transaction(async (trx) => {
            const [newTask] = await trx('tasks')
                ...
                .returning(['id', 'title', 'description', 'priority', 'status', 'due_date', 'assigned_to', 'project_id', 'created_at']);
...
export const getTaskById = async (req,res,next) => {
    try {
        ...
        if(role !== 'admin' && role !== 'manager'){
            return res.status(403).json({message:"Ushbu amaliyotni bajarish uchun sizda ruxsat yo'q.Tez borib ruhsat olib keling."})
        }

        const task = await db('tasks')
        .select('id','title','description','status','priority','due_date','assigned_to','project_id','created_at','updated_at')
        .where({ id })
        .first();
...
export const updateTask = async (req, res, next) => {
    try {
        ...
        .returning(['title', 'description', 'status','priority','due_date','assigned_to','project_id', 'updated_at'])
